...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Exploit", async function () {
    ...
    // Calculate the initial deposit needed
    ...
    // Calculate the maximum output amount
    const amountOut = await this.uniswapRouter.getAmountOut(
    ...
    // Make a swap to unbalance the pool
    ...
    // Calculate the new deposit needed
    ...
    // Approve needed WETH to lending pool
    await this.weth.connect(attacker).approve(this.lendingPool.address, deposit);

    // Borrow all token balance from lending pool
